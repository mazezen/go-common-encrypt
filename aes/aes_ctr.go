...
// NewAesCTR bit-words contains **AES-128**, **AES-192**, **AES-256**
// Fill method include
// - PKCS#7
// - ZeroPadding
// - NoPadding
func NewAesCTR(k []byte, args ...string) *AesCTR {
	b, err := aes.NewCipher(k)
	if err != nil {
		panic(fmt.Errorf("NewAesCTR failed: [%+w]", err))
	}
	...
	return &AesCTR{base: base{key: k, block: b, fm: fillMode}}
}

func (this *AesCTR) AesCTREncrypt(plainText []byte) (string, error) {
	...
	mode := cipher.NewCTR(this.block, iv)
	encrypted := make([]byte, len(padded))
	mode.XORKeyStream(encrypted, padded)
...
func (this *AesCTR) AesCTRDecrypt(cipherText string) (string, error) {
	...
	mode.XORKeyStream(decrypted, cipherBytes)
